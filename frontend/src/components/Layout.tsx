...
const navigation = [
  ...
  { name: 'Satış',          path: '/sales',         icon: '◈'              },
  { name: 'Satış Geçmişi',  path: '/sales/history', icon: '≡'              },
  { name: 'Siparişler',     path: '/orders',        icon: '◻'              },
...
export default function Layout({ children, user, onLogout }: LayoutProps) {
  ...
  const currentPage = navigation.find(n => isActive(n.path, n.exact))?.name ?? 'Windoor';
    ...
    <>
      ...
      <div className="px-5 py-5" style={{ borderBottom: '1px solid var(--sidebar-border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>Stok Yönetim</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4" style={{ borderTop: '1px solid var(--sidebar-border)' }}>
        ...
        >
          ...
          <span className="font-medium">{dark ? 'Açık Tema' : 'Koyu Tema'}</span>
        </button>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top bar */}
        <header
          className="flex items-center justify-between px-4 lg:px-8 py-3 flex-shrink-0"
          style={{
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span
                ...
                style={{ background: 'var(--text)', borderRadius: '2px' }}
              />
              <span
                className="block w-5 h-0.5 mb-1 transition-all"
                style={{ background: 'var(--text)', borderRadius: '2px' }}
